...
SpriteUI::SpriteUI()
{
	...
	m_spriteUI[enSpriteUI_kakeru].Init(L"Assets/sprite/kakeru.dds", 30, 30);
...
void SpriteUI::Update()
{
	...
	if (m_game != nullptr) {
		auto ten = m_game->GetScore() % 100;
		auto Hundred = m_game->GetScore() % 1000;
		//値に応じて表示・非表示切り替え
